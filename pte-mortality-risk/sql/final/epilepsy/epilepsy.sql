...
SELECT
    dx.PatientICN,
    CASE
        WHEN MIN(asd.FillDate) >= MIN(dx.DxDate) THEN MIN(asd.FillDate)
        ELSE MIN(dx.DxDate)
    END AS DxDate
INTO
    #Criteria1
...
ValidDates AS (
    ...
    INNER JOIN
        FirstYear fy
        ON
        dx.PatientICN = fy.PatientICN
        AND
        YEAR(dx.DxDate) = fy.FirstValidYear
),
RankedDates AS (
    SELECT
        *,
...
DROP TABLE IF EXISTS ORD_Haneef_202402056D.Dflt.rnEpilepsyFinal;
CREATE TABLE ORD_Haneef_202402056D.Dflt.rnEpilepsyFinal (
...
INSERT INTO ORD_Haneef_202402056D.Dflt.rnEpilepsyFinal (
    PatientICN,
    DxDate
)
SELECT
...
FROM 
    ...
    #SeizureDx dx
        ON
        merged.PatientICN = dx.PatientICN
GROUP BY
    merged.PatientICN
;

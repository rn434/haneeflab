...
epilepsy according to Haneef 2022.
The time period extends from October 1, 2013 to March 31, 2020.
...
Criteria:
    ...
    (1) At least one documented seizure-related ICD code in the index year or 
        ...
        the index year. Gabapentin only counts if it was accompanied by ICD-9
        345 or ICD-10  G40.*, R56.1, or R56.9.
    ...
    way to gauge inpatient (D, H, I, O) vs outpatient (A, R, S, T, X)
...
SELECT coh.PatientICN,
    ...
    CASE WHEN asm.DrugNameWithoutDose LIKE 'gabapentin' THEN 1 ELSE 0 END AS GabapentinFlag
INTO #ASMFills
...
WHERE fill.DaysSupply >= 30
    AND FillDateTime BETWEEN '2013-10-01' AND '2020-03-31';
...
WHERE (s1.StopCodeSID IS NULL AND s2.StopCodeSID IS NULL)
    ...
    AND VDiagnosisDateTime BETWEEN '2011-10-01' AND '2020-03-31';
...
WHERE (icd9.ICD9SID IS NOT NULL OR icd10.ICD10SID IS NOT NULL)
    AND COALESCE(icd9.DxType, icd10.DxType) LIKE 'epilepsy'
    AND DischargeDateTime BETWEEN '2013-10-01' AND '2020-03-31';
...
WITH DiagnosisCounts AS (
    ...
    WHERE DxType = 'Epilepsy'
        AND DxDate BETWEEN '2013-10-01' AND '2020-03-31'
    GROUP BY PatientICN,
        Year(DxDate)
...
DROP TABLE IF EXISTS SCS_EEGUtil.EEG.rnRehmanEpilepsy;
WITH MergedCriteria AS (
    SELECT PatientICN, DxDate FROM #Criteria1
    UNION ALL
    SELECT PatientICN, DxDate FROM #Criteria2
    UNION ALL
    SELECT PatientICN, DxDate FROM #Criteria3)
, PatientASMCounts AS (
    SELECT PatientICN,
        COUNT(DISTINCT DrugNameWithoutDose) AS DistinctASMCount
    FROM #ASMFills
    WHERE DrugNameWithoutDose NOT LIKE 'gabapentin'
    GROUP BY PatientICN
)
SELECT merged.PatientICN,
    MIN(merged.DxDate) AS DxDate,
    COALESCE(asm.DistinctASMCount, 0) AS DistinctASMCount
INTO SCS_EEGUtil.EEG.rnRehmanEpilepsy
FROM MergedCriteria merged
    LEFT JOIN PatientASMCounts asm
        ON merged.PatientICN = asm.PatientICN
GROUP BY merged.PatientICN, asm.DistinctASMCount;
...
SELECT COUNT(*) AS AllEpilepsyCount FROM SCS_EEGUtil.EEG.rnRehmanEpilepsy;
SELECT COUNT(*) AS DRECount from SCS_EEGUtil.EEG.rnRehmanEpilepsy WHERE DistinctASMCount >= 2;
